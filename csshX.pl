...
use version; our $VERSION = qv("0.72");
(our $REVISION = '$Rev: 146 $') =~ s/\D//g;
...
my @config_keys = qw(
    ...
    tile_x tile_y ssh_args no_growl
    ...
    session_max ping_test ping_timeout ssh
);
...
sub new {
    ...
    GetOptions($obj, 
        ...
        "screen=i",    "space=i",      "ssh_args=s",     "debug:+",
...
sub hosts {
    ...
    while (my $arg = shift @ARGV) {
        if (my $clusthosts = $obj->{clusters}->{$arg}) {
            ...
            if ($ping) {
                my ($user, $host, $port) = CsshX::Process->parse_user_host_port($arg);
...
sub physical_screen_bounds { 
    ...
    my $frame = NSScreen->screens->objectAtIndex_($scr)->visibleFrame;
    return [ObjCStruct::NSRect->unpack($frame)];
}
...
sub set_space {
    ...
    $obj->run_ruby("
        ...
        dl.sym('CGSMoveWorkspaceWindowList', 'IIAII').call(con[0], [Integer(ARGV[0])], 1, Integer(ARGV[1]))
    ", $obj->windowid, $space);
}
...
sub space {
    ...
    $obj->run_ruby("
        ...
        dl = DL::dlopen('/System/Library/Frameworks/ApplicationServices.framework/ApplicationServices')
        con = dl.sym('_CGSDefaultConnection', 'I').call()
        r,rs = dl.sym('CGSGetWindowWorkspace', 'IIIi').call(con[0], Integer(ARGV[0]), 0)
        exit rs[2]
    ", $obj->windowid);
}
...
sub new {
    ...
    $greeting .= $master->uid."\n";
    $master->set_space($config->space) if $config->space;
...
sub terminate {
    Growl->notify("Close", "Closed ".$config->slavehost);
    $_[0]->SUPER::terminate;
}
...
my $modes = {
    'input' => {
        prompt =>  sub { "Input to terminal: (Ctrl-a to enter control mode)\r\n" },
    ...
    'action' => {
        prompt => sub { 
            ...
            "Actions (Esc to exit, Ctrl-a / a to send Ctrl-a to input)\r\n".
        ...
        parse_buffer => sub {
            ...
            while (length $buffer) {
                ...
                if ($buffer =~ s/^(\014|\e\[5C)//) {
                    ...
                    $obj->master->move_slaves_to_master_space();
    ...
    'enable' => {
        prompt => sub { "Select window with Arrow keys of h,j,k,l: (Esc to exit)\r\n".
        "[e]nable input, [d]isable input, disable [o]thers, diable [O]thers and zoom, [t]oggle intput" },
...
# Workaround for boolean ObjCBridge bug in early 10.6!! rdar://problem/7886872
...
elsif ($config->version) { die sprintf "csshX $VERSION-$REVISION\n", $VERSION }
...
You can also use hostsname ranges, to specify many hosts.
...
Sets the screen on which to display the terminals, if you have multiple 
monitors.

=item B<--space> I<number>

Sets the space (if spaces are enabled) on which to display the terminals.
...
Change the command that is run. May useful if you use an alternative ssh binary
...
single csshX session. By default csshX will not open more than 256 sesions.
You must set this to something really high to get around that. (default: 256)

Note that you will probably run out of Pseudo-TTY's before reaching 256
terminal windows.

=item B<--ping_test>, B<--ping> I<number>

To avoid opeining connections to machines that are down, or not running ssh, 
...
Use of this option is highly recomended when subnet ranges are used.
...
If you have a lot of similarly named hosts, or which to open all hosts in
a subnet, hostname ranges will simplify things. Howerver this also allows
to open crazy numbers of windows. To save you from yourself, B<session_max>
will limit the number of hosts opened.

It is also recomended to enable B<ping_test> if only a few machines on a 
...
Not all Cluster-SSH attributes are supported, and a few attribute have
been added.
...
Color specifcations can be in two formats. Applescript format consists of
three integer values in the form B<{ nnnnn,nnnnn,nnnnn }>, where nnnnn's are
red, green and blue decimal values between 0 and 65535. Web style 
B<HHHHHH> style where HH's are red, green and blue hex values between
00 and FF (note the leading # is not used to avoid confusion with comments).
...
(csshX only) The applescript color identifier for the foreground font in
bounds setting mode. Default: Nothing
